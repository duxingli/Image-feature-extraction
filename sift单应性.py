...
# img0 = cv.imread('./images/3.jpg')
# img1 = cv.imread('./images/0.jpg')
# 转换为灰度图像
# img0 = cv.cvtColor(img0, code=cv.COLOR_BGR2RGBA)
# img1 = cv.cvtColor(img1, code=cv.COLOR_BGR2RGBA)
# img0 = addGaussNoise(img0, mean=0, var=0)
# img1 = addGaussNoise(img0, mean=0, var=0)
...
if len(good_matches) >= MIN_NUM_GOOD_MATCHES:
    ...
    # rotate = cv.warpPerspective(img1, M1, img1.shape)
    # plt.imshow(rotate)
    # plt.show()
